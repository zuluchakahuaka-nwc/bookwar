...
(async () => {
  ...
  await p.evaluate(() => window.gameTestGotoMap('stony_wastes'));
  for (let k = 0; k < 24; k++) { await sleep(500); const r = await p.evaluate(() => (window.gameHUD || {}).region || ''); if (r && r !== prev && r !== '?') break; }
  await sleep(1500);
  // Нейтрализовать всех монстров чтобы избежать случайного боя
  ...
  await sleep(1200);
  await p.screenshot({ path: 'D:/Projects/BOOKWAR/tests/screenshots/quest_log_5quests.png' });
