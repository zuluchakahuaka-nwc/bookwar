...
## 2026-07-08 (бонус) — E2E walkthrough одной карты (12)

### Что сделано
- ✅ **`walkthrough_map12.js`** — полноценный e2e бот для одной карты (Туманная Роща)
- ✅ Проверены все этапы gameplay:
  - Загрузка карты → HUD, lore, progress bar видны
  - Журнал квестов открывается (10 квестов на карте 12)
  - Экран статистики открывается (HP, буквы, квесты, сила)
  - F-атака на врага → battle_scene запускается корректно
  - **`gameAutoBattle()`** JS bridge → автобой завершается автоматически
  - `gameClearRegion()` → все монстры нейтрализованы → victory=true
  - **Портал спавнится** при прогрессе ≥50% (bot получил 53%)
  - **Переход через портал** → следующая карта «Серый Лес» (13)
- ✅ 11 скриншотов в `tests/screenshots/walk_map12/`

### Технические детали
- Враги: 19 hostile + 11 нейтральных = 30 всего
- После боя HP игрока: 100 → 50 (потери в боях)
- Прогресс 53% после clearRegion (портал открывается при ≥50%)
- Vision подтвердил: «ПОБЕДА! Туманная Роща зачищена» + портал виден

### Bug найден при тесте
- `gameTriggerDialogue()` не открывает диалог если игрок в battle_scene
- Workaround в боте: убедиться что мы в world_map перед trigger
- Реальный игрок не столкнётся (T-key работает только в мире)

### Вывод
Карта 12 полностью проходима: от старта до портала на следующую.
Все системы (квесты, бой, прогресс, порталы) работают в связке.
